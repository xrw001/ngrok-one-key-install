...
if [ ! -s /usr/local/ngrok/bin/ngrokd ]; then
    ...
    echo -e "Please run \033[32m\033[01mwget --no-check-certificate https://github.com/xrw001/ngrok-one-key-install/raw/master/ngrok_install.sh && bash ./ngrok_install.sh\033[0m install ngrok."
    exit 1
fi
...
function start_ngrok_clang(){
    ...
    if [ "${str_Ngrok_PID}" = "" ]; then
        ...
        nohup ./bin/ngrokd -domain="$dns" -httpAddr=":$http_port" -httpsAddr=":$https_port" -pass="$pass" -tlsCrt="$srtCRT" -tlsKey="$strKey" -tunnelAddr=":$remote_port" -log-level="INFO" > ${ngrok_log} 2>&1 &
